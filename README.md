...
42        719s    alive    7180      28        1
1337      480s     died    3183      17        0
99        720s    alive    6055      25        1
7         720s    alive    6918      24        1
11        720s    alive    2552      18        1
2024      702s     died    3133      21        0
5         719s    alive    7528      29        1
808       415s     died    1577      13        0

дошли до босса: 6/8   свалили хотя бы одного: 5/8   средний забег: 649s
```

Снята после сундуков и по ним же настроена. Сундуки — подарок, они обязаны
двигать таблицу вверх, вопрос был только насколько:

| | без сундуков | **интервал 150** | интервал 210 |
|---|---|---|---|
| дошли до босса | 5/8 | **6/8** | 5/8 |
| свалили хотя бы одного | 4/8 | **5/8** | 4/8 |
| средний забег | 592s | **649s** | 620s |
| живы на звонке | 4/8 | **5/8** | 4/8 |

При 210 секундах сундуки почти не читаются: +28 секунд к среднему забегу и ни
одной сдвинутой колонки, при том что за каждый сундук игрок платит крюком через
уже пройденную землю. 150 даёт видимую помощь и оставляет игру проигрываемой —
трое из восьми всё ещё умирают внутри окна.

Первый раз этот же A/B был снят до копья и гарпуна, и дал ровно обратный ответ:
на трёх оружиях 150 секунд означали **8/8** до босса, 714s и ни одной смерти
внутри окна, а правильным выглядел интервал 210. Два новых оружия и новый враг
сдвинули базу настолько, что настройка, сделанная под старую игру, оказалась
перекручена в другую сторону. Мораль ровно та же, что абзацем выше про кольцо
лезвий: цифры в этом файле стоят ровно столько, сколько стоит последняя
прогонка.
...
загрузится. Запасной путь работает покадрово, а не «всё или ничего»: три кадра
остаются нарисованными даже когда всё вокруг пришло с листа, и каждый по своей
причине. У ударной волны в подземельном тайлсете нет эквивалента. Копьё и гарпун
разбираются ниже.
