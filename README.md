...
оружие. Цветом два выстрела не различить: как только загружается артворк,
`variantTint` возвращает белый, потому что тайл несёт собственный цвет. Различает
их форма, поэтому кадр привязан к оружию, а не к пулу, из которого снаряд достали.
