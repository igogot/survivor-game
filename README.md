...
выбор апгрейда, кнопки паузы и перезапуска появляются сами, когда клавиатуры нет.
...
- **Две кнопки, которых нет на десктопе** — пауза и «сыграть ещё». Они
  появляются только при `pointer: coarse`, так что клавиатурные экраны остались
