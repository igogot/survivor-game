...
export class GameRenderer {
  ...
  private drawFlames(world: World): void {
    ...
    for (let i = 0; i < flames.length; i++) {
      ...
      // Not `variantTint`: there is no fire tile in the artwork, so this frame
      // is a drawn white mask whichever texture set loaded and always has to be
      // tinted. The shockwave ring and the lance are the same case.
      sprite.tint = flame.color;
