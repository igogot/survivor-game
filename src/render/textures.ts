...
import { drawGround, loadTownSheet } from './ground';
...
export async function createTextures(): Promise<TextureSet> {
  ...
  const ground = Texture.from(await loadGround());
...
/**
 * The land, or the empty grid this game shipped with.
 *
 * Two sheets can fail independently, and they should: a run with no ground art
 * is still a run, and one with no sprite art is still a run. Neither is
 * allowed to take the other down with it.
 */
async function loadGround(): Promise<HTMLCanvasElement> {
  try {
    return drawGround(await loadTownSheet());
  } catch (error) {
    console.warn('Ground artwork unavailable; falling back to an empty grid.', error);
    return drawToCanvas(GRID_TEXTURE_SIZE, GRID_TEXTURE_SIZE, drawGrid);
  }
}
