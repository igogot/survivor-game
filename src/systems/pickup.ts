...
import { healthScale, nearestPlayer } from '../world/party';
 ...
 *
 * What it pays is multiplied by the *health* the party gave the thing that
 * dropped it, which is the general form of "an enemy worth four times the
 * shooting is worth four times the experience". It has to be the health scale
 * and not the party's size: a party that meets its multiplier as more bodies
 * rather than tougher ones already collects that many more gems, and paying
 * again per gem would be the same double count in the other direction.
 *
 * Without it half of the pair of party multipliers charges and the other half
 * does not pay, and a party levels at half a solo player's pace. The stand
 * measured exactly that: a duo felled no boss in eight seeds where a solo
 * player felled six.
 *
...
export function pickupSystem(world: World, dt: number): void {
  ...
  for (let i = gems.length - 1; i >= 0; i--) {
    ...
    if (distanceSq <= collectRadiusSq) {
      world.xp += gem.value * healthScale(world);
